...
公司资料独立保存在 `yellow/`。具体操作由网站门户手册说明。
...
OMP 基于上游项目 Fork 持续维护；Fork 自身的开发独立进行。本效率仓库不修改 OMP 源码。更新二进制只运行一个 Python 脚本，不要求克隆源码或更新元数据。手册、Skill、命令及 Worker 按需单独维护，说明标注参考版本；
需要核验实现时可在 `.tmp/` 只读参考 Fork 源码 https://github.com/jchanghong023/oh-my-pi。

目标使用环境为 **CentOS 7**，OMP 与 Claude Code 二进制兼容性以用户已确认结果为前提。同步、维护、构建、打包和验证脚本统一使用目标机器已安装后的 **Python 3.11+**；普通用户浏览已生成门户不需要网站构建环境。

普通用户执行安装脚本需要**Python 3.11+**。

能力分为两层：

| 层次 | 内容 |
|---|---|
| OMP 内置能力 | 公司内部模型接入和认证接入、Markdown Wiki、全文索引、内置命令和工具等 |
