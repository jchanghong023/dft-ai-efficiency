...
3. `Tperiod - Tsetup - Tskew` 是本实验用来估算的简化可用窗口。这里 `Tskew ≥ 0` 表示按不利方向扣除的时钟偏斜预算，不是带符号的 `capture 时钟到达时间 − launch 时钟到达时间`；按后一种定义，正偏斜反而增加 setup 可用时间。真实签核还要考虑时钟树、OCV、脉宽、库模型和测试约束。

输入的“路径延迟”包含发射寄存器的 clock-to-Q、组合逻辑和互连延迟，计至捕获寄存器的数据端；捕获端建立时间在公式中单独扣除。

这里的“通过”只表示输入参数满足这个教学不等式；LOS 还要求 `SE` 在 capture 前留出本实验复用的建立裕量：

```text
路径延迟 + 不利时钟偏斜预算 ≤ 时钟周期 − 捕获端建立时间
```
